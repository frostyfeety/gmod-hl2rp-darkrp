...
local function textPlate(text,y)
	...
	local bx,by = -tw / 2 - 5, y - 5
	local bw,bh = tw + 5 + 5, th + 5 + 10
...
local function drawInfo(ent, text, dist)
	...
	if dist < 60000 then
		...
		cam.Start3D2D(center + Vector(0, 0.2, math.abs(max.z / 2) + 7 + sin), Angle(0, rot, 90), 0.13)
			textPlate(text,15)
		cam.End3D2D()
...
function ENT:Draw()
	...
	self:DrawModel()
	self:SetSequence(3)
	drawInfo(self, "Гражданский Союз Рабочих", dist)
end