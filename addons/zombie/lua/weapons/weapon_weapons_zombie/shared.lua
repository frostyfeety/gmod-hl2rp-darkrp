...
hook.Add('canDropWeapon', 'rp_zombie', function(ply, wep)
	if IsValid(wep) and wep:GetClass() == 'weapon_weapons_zombie' then
		return false
	end
end)

hook.Add('onDarkRPWeaponDropped', 'rp_zombie', function(ply, spW, orW)
	if IsValid(orW) and orW:GetClass() == 'weapon_weapons_zombie' then
		spW:Remove()
	end
end)
