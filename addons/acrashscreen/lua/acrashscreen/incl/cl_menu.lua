...
function PANEL:Init()
	...
	self.reconnectButton:SetText( "Перезайти сейчас" )
	...
	self.terminateButton.DoClick = function( pnl )
		...
		label.Paint = function( pnl, w, h )
			...
			local tw, th = surface.GetTextSize( "Вы уверены?" )
...
function PANEL:Paint( w, h )
	...
	if self.reconnectingType == TYPE_WAITFORSERVER then
		...
		if self.isServerOnline then
			...
			draw.SimpleText(
				"Сервер вернулся обратно", 'acrashscreen_big',
			...
			draw.SimpleText(
				"Вы будете автоматически подключены через " .. self.reconnectingInTime .. " секунд.", 'acrashscreen_small',
