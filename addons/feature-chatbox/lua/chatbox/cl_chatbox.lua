...
cvars.AddChangeCallback("lounge_chat_hidetime", function(cvar, old, new)
	chat_message_hidetime_cache = tonumber(new) or 2
end)

--
local matClose = Material("icon16/cross.png", "noclamp smooth")
...
function LOUNGE_CHAT:CreateChatbox()
	...
	frame.Paint = function(me, w, h)
		...
		if (self.ChatboxOpen or me.m_fAlpha > 0) then
			...
			draw.RoundedBoxEx(0, 0, 0, w, 22, Color(ch.r, ch.g, ch.b, ch.a * af), true, true, false, false)
			draw.RoundedBoxEx(0, 0, 22, w, h - 22, Color(cb.r, cb.g, cb.b, cb.a * af), false, false, true, true)
	...
	_LOUNGE_CHAT = frame
		...
		close.Paint = function(me, w, h)
			if (me.Hovered) then
				draw.RoundedBoxEx(0, 0, 0, w, h, self.Color("close_hover"), false, true, false, false)
			end

			if (me:IsDown()) then
				draw.RoundedBoxEx(0, 0, 0, w, h, self.Color("hover"), false, true, false, false)
			end
		...
		history.Paint = function(me, w, h)
			if (self.ChatboxOpen) then
				...
				draw.RoundedBox(0, 0, 0, w, h, Color(ci.r, ci.g, ci.b, ci.a * af))
		...
		frame.m_Bottom = bottom
			...
			entry.Paint = function(me, w, h)
				draw.RoundedBoxEx(0, 0, 0, w, h, self.Color("inbg"), true, false, true, false)
