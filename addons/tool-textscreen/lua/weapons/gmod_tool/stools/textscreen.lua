...
if (CLIENT) then
	...
	language.Add("tool.textscreen.name", "Текст-скрины")
	language.Add("tool.textscreen.desc", "Создать текст-скрин с цветом, размером и шрифтом.")
	language.Add("tool.textscreen.left", "Создать текст-скрин") -- Does not work with capital T in tool. Same with right and reload.
	language.Add("tool.textscreen.right", "Обновить текст-скрин.")
	language.Add("tool.textscreen.reload", "Скопировать текст-скрин.")
	language.Add("Undone.textscreens", "Текст-скрин удален")
	language.Add("Undone_textscreens", "Текст-скрин удален")
	language.Add("Cleanup.textscreens", "Текст-скрины")
	language.Add("Cleanup_textscreens", "Текст-скрины")
	language.Add("Cleaned.textscreens", "Все текст-скрины очищены")
	language.Add("Cleaned_textscreens", "Все текст-скрины очищены")
	language.Add("SBoxLimit.textscreens", "У вас лимит текст-скринов!")
	language.Add("SBoxLimit_textscreens", "У вас лимит текст-скринов!")
end

function TOOL:LeftClick(tr)
	if IsValid(tr.Entity) and tr.Entity:GetClass():lower() == "player" then return false end
	if IsValid(tr.Entity) and tr.Entity:GetClass():lower() == "func_door" then return false end
	if IsValid(tr.Entity) and tr.Entity:GetClass():lower() == "prop_dynamic" then return false end
	if IsValid(tr.Entity) and tr.Entity:GetClass():lower() == "prop_door_rotating" then return false end
	if IsValid(tr.Entity) and tr.Entity:GetClass():lower() == "func_movelinear" then return false end
...
function TOOL.BuildCPanel(CPanel)
	...
	cvars.AddChangeCallback("textscreen_font1", function(convar_name, value_old, value_new)
		...
		changefont:SetText("Сменить шрифт (" .. font .. ")")
	end)
	...
	changefont:SetText("Сменить шрифт (" .. TrimFontName(fontnum) .. ")" )
