...
TOOL.ClientConVar[ "ropelength" ] = "0"
...
function TOOL:LeftClick( trace, attach )
	...
	local brght = math.Clamp( self:GetClientNumber( "brightness" ), 0, 2 )
	local size = math.Clamp( self:GetClientNumber( "size" ), 0, 500 )
	...
	undo.Create( "Light" )
		...
		if ( attach ) then
			...
			local length = math.Clamp( self:GetClientNumber( "ropelength" ), 3, 4 )
